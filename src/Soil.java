...
public class Soil extends SoilCell {
    ...
    public void updateWater(Forest forest, SoilCell[][] newSoilField) {
        ...
        double infiltrationLoss = this.type.getPermeability() * newSoil.waterContent;
        newSoil.waterContent -= infiltrationLoss;
        if (newSoil.waterContent < 0) {
            newSoil.waterContent = 0;
        }

        if (newSoil.waterContent > this.type.getCapacity()) {
            double surplus = newSoil.waterContent - this.type.getCapacity();
