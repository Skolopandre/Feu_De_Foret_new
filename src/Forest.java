...
public class Forest {
    ...
    public boolean hasWaterNeighbor(int x, int y) {
        ...
        for (Cell neighbor : getNeighbors(x, y)) {
            if (neighbor != null &&
                    neighbor.getType() == CellType.WATER) {
    ...
    public void addRain(int nx,int ny,double rainAmount){
        soilField[nx][ny].setWaterContent(rainAmount,0);
    }

    public void updateAtmosphericHumidityAt(int x,int y){
        if(getCellAt(x,y).getType() == CellType.TREE){
            atmosphericHumidity[x][y]+=getSoilFieldAt(x,y).getWaterContent();
        }
        if(atmosphericHumidity[x][y]>=1){
            spawnCloud(x,y);
        }
    }
    ...
    public void initializeWorld() {
        ...
        /*
        // Biomes WATER
        int numWaterBiomes = (this.width+this.height)/40;
        for (int i = 0; i < numWaterBiomes; i++) {
            ...
            waterBiome.generateFrom(grid, x_random, y_random,this);
        }
